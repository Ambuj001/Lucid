      ...
      <div className="w-64 bg-white border-r border-[#E9ECEF] p-6 flex flex-col justify-between">
        <div>
          <div className="flex items-center gap-3 mb-10 pl-2">
            <div className="w-8 h-8 bg-blue-600 rounded-xl flex items-center justify-center text-white font-black text-sm shadow-md shadow-blue-200">L</div>
        ...
        {activeTab === 'portfolio' && (
          <div className="max-w-4xl space-y-8">
            ...
            {myWalletCards.length === 0 ? (
              <div className="bg-white border border-dashed border-[#E9ECEF] p-12 rounded-2xl text-center text-sm text-zinc-400">
                NO ASSETS CURRENTLY PROVISIONED IN WALLET. USE THE SEARCH BLOCK ABOVE TO MOUNT CARDS.
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {myWalletCards.map((card: any) => (
                  <div key={card.wallet_entry_id} className="bg-white border border-[#E9ECEF] p-6 rounded-2xl shadow-sm flex flex-col justify-between h-44 hover:shadow-md transition-shadow">
                    <div>
                      <div className="flex justify-between items-center text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
                        ...
                        <span className="bg-zinc-100 px-2 py-0.5 rounded-md text-zinc-600">{card.card_network}</span>
                      </div>
                      <h4 className="text-lg font-bold text-zinc-900 mt-3 leading-tight">{card.card_name}</h4>
                      <p className="text-[10px] text-zinc-400 mt-1 uppercase font-bold tracking-tight">Tier: {card.card_type}</p>
                    </div>
                    <div className="text-xs text-blue-600 font-semibold flex items-center justify-between border-t border-zinc-50 pt-3">
                      <span>Status: Core Active Map</span>
                      <span className="text-[10px] text-zinc-400">ID: {card.card_id.replace('in_', '')}</span>
                    </div>
                  </div>
                ))}
              </div>
